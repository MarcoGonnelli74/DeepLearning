...
int main(int argc, char** argv)
{
    ...
    // Create a window
    static const string kWinName = "Mask R-CNN Object Detection and Segmentation";
    ...
    while (waitKey(1) < 0)
    {
        ...
        //string label = format("Mask-RCNN on NVIDIA RTX 2080 GPU , Inference time for a frame : %0.0f ms", t);
        string label = format("Mask-RCNN on NVIDIA RTX 2080 GPU , Fps : %0.0f ", fps);
...
void drawBox(Mat& frame, int classId, float conf, Rect box, Mat& objectMask)
{
    ...
    //cout << "BOX -> X:" << box.x << " Y:" << box.y << " W:" << box.width << " H:" << box.height << endl;
    //cout << "FRAME -> W:" << frame.cols << " H:" << frame.rows << endl;

    // Fit BOX to Frame
    if(box.width + box.x > frame.cols)
    {
    	box.width = frame.cols - box.x -10;
    }

    if(box.height + box.y > frame.rows)
    {
    	box.height = frame.rows - box.y -10;
    }

    //cout << "BOX POST FIT -> X:" << box.x << " Y:" << box.y << " W:" << box.width << " H:" << box.height << endl;
